...
def skew(df: pd.DataFrame, feature_store: FeatureStore):
    ...
    numerical_columns = _get_numerical(df)
    pt = PowerTransformer()
    transformed = pt.fit_transform(numerical_columns)
    numerical_columns_df = pd.DataFrame(transformed, columns=numerical_columns.columns, index=numerical_columns.index)
    df.loc[:, numerical_columns.columns] = numerical_columns_df
    return df